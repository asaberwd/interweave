...
var EMOJI_REGEX = _interopDefault(require('emojibase-regex'));
var EMOTICON_REGEX = _interopDefault(require('emojibase-regex/emoticon'));
var SHORTCODE_REGEX = _interopDefault(require('emojibase-regex/shortcode'));
...
var EmojiDataManager = function () {
  ...
  _proto.packageEmoji = function packageEmoji(baseEmoji) {
    ...
    if (!emoji.unicode) {
      emoji.unicode = emoji.text && emoji.type === emojibase.TEXT ? emoji.text : emoji.emoji;
    }
    ...
    if (emoticon) {
      emojibase.generateEmoticonPermutations(emoticon, emojibase.EMOTICON_OPTIONS[emoticon]).forEach(function (emo) {
        _this.EMOTICON_TO_HEXCODE[emo] = hexcode;
      });
    }

    this.UNICODE_TO_HEXCODE[emojibase.fromCodepointToUnicode(emojibase.fromHexcodeToCodepoint(hexcode))] = hexcode;
...
function Emoji(_ref) {
  ...
  if (!hex || !data.EMOJIS[hex]) {
    return React__default.createElement("span", null, unicode || emoticon || shortcode || hex);
  }

  var emoji = data.EMOJIS[hex];

  if (renderUnicode) {
    return React__default.createElement("span", null, emoji.unicode);
  }
...
var EmojiMatcher = function (_Matcher) {
  ...
  _proto.replaceWith = function replaceWith(children, props) {
    return React__default.createElement(Emoji, _extends({}, props, {
  ...
  _proto.onAfterParse = function onAfterParse(content, props) {
    ...
    return content.map(function (item) {
      ...
      var element = item;
      return React__default.cloneElement(element, _extends({}, element.props, {
        enlargeEmoji: true
      }));
    });
  };
...
function loadEmojis(locale, version, compact) {
  ...
  if (stubRequest) {
    ...
    request = emojibase.fetchFromCDN(locale + "/" + set + ".json", version);
  }
...
exports.Emoji = Emoji;
exports.EmojiDataManager = EmojiDataManager;
